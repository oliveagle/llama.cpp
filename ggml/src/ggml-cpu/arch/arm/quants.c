...
void ggml_vec_dot_q1_0_g128_q8_0(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int k = 0; k < 4; k++) {
            ...
            const int8x16_t y0 = vld1q_s8(y[i*4 + k].qs);
            const int8x16_t y1 = vld1q_s8(y[i*4 + k].qs + 16);

            // Bit offset into the 128-bit qs array: k * 32 bits = k * 4 bytes
            const uint64_t expand0 = table_b2b_0[bits[k*4 + 0]];
            const uint64_t expand1 = table_b2b_0[bits[k*4 + 1]];
            const uint64_t expand2 = table_b2b_0[bits[k*4 + 2]];
            const uint64_t expand3 = table_b2b_0[bits[k*4 + 3]];
    ...
    sumf = vaddvq_f32(sumv);
#else
    ggml_vec_dot_q1_0_g128_q8_0_generic(n, &sumf, bs, vx, bx, vy, by, 1);
#endif
